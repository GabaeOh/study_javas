            ...
            while(!workkey.equals("E")){
                ...
                if (workkey.equals("O")){
                    ...
                    while (resultSet.next()) {
                        ...
                        carNumberMap.put(String.valueOf(number), carInforId); // 해쉬맵에 값을 넣는 put / valueOf = 넘버를 스트링으로 바꿔주는/ key : 가상의 순번, value : pk 
                        ...
                        query2 = "SELECT T_OPT_INFOR.OPTION_NAME\n" + //
                                ...
                                "    AND T_OPTS.CAR_INFOR_ID = '"+ resultSet.getString("CAR_INFOR_ID")+"'";  // 변수을 넣을 때는 '"+ +"' 안에 넣기 
                    ...
                    number = 1;
                    HashMap<String, String> carOptionInfor = new HashMap<>(); // 순번과 옵션pk(OPTION_INFOR_ID)를 연결 // KEY 랑 VALUE값을 연결시켜주기 위해 해쉬맵 사용
                    while(resultSet.next()){ // 번호와 옵션명을 출력
                        System.out.print(number +", "+resultSet.getString("OPTION_NAME")+",");
                        carOptionInfor.put(String.valueOf(number), resultSet.getString("OPTION_INFOR_ID")); // 순번과 옵션 pk연결하여 값 추가 
                    ...
                    System.out.println(carNumberMap.get(CarNumer) +","+ carOptionInfor.get(optionNumber)  ); //CAR_INFOR_ID과  옵션 PK(OPTION_INFOR_ID)를 연결, 원하는 옵션을 해당차(CAR_INFOR_ID) 에 정보를 넣어줘야 되서 
